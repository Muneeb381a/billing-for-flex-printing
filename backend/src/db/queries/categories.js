...
export const findAll = () =>
  pool.query(
    `SELECT c.id, c.name, c.slug, c.description, c.pricing_type, c.rate, c.unit,
...
export const create = ({ name, slug, description = null, pricingType = 'area_based', rate = null, unit = 'sqft', minSqft = 1, sortOrder = 0 }) =>
  pool.query(
    `INSERT INTO categories (name, slug, description, pricing_type, rate, unit, min_sqft, sort_order)
     VALUES ($1,$2,$3,$4,$5,$6,$7,$8)
     RETURNING *`,
    [name, slug, description, pricingType, rate, unit, minSqft, sortOrder]
  );

export const update = (id, { name, slug, description, isActive, pricingType, rate, unit, minSqft, sortOrder }) =>
  pool.query(
    `UPDATE categories
     SET    name         = COALESCE($2, name),
            ...
            min_sqft     = COALESCE($9, min_sqft),
            sort_order   = COALESCE($10, sort_order)
     WHERE  id = $1
     RETURNING *`,
    [id, name, slug, description, isActive, pricingType, rate, unit, minSqft, sortOrder]
  );
